        ...
        try:
            ...
            for pcd in self.InfPcdList:
                ...
                if (cmp (pcd[4], "BOOLEAN") == 0) or (cmp (pcd[4], "UINT8") == 0):
                    ...
                    print ("  [" + hex(offset) + "] " + binascii.hexlify(data[offset:offset+1]).decode('utf-8')  + " <= " + str(binascii.hexlify(b)))
                    data[offset:offset+1] = b
                elif (cmp (pcd[4], "UINT16") == 0):
                    h = struct.pack("H", int(pcd[3],16))
                    print ("  [" + hex(offset) + "] " + binascii.hexlify(data[offset:offset+2]).decode('utf-8')  + " <= " + str(binascii.hexlify(h)))
                    data[offset:offset+2] = h
                elif (cmp (pcd[4], "UINT32") == 0):
                    l = struct.pack("I", int(pcd[3],16))
                    print ("  [" + hex(offset) + "] " + binascii.hexlify(data[offset:offset+4]).decode('utf-8') + " <= " + str(binascii.hexlify(l)))
                    data[offset:offset+4] = l
                elif (cmp (pcd[4], "UINT64") == 0):
                    q = struct.pack("Q", int(pcd[3],16))
                    print("  [" + hex(offset) + "] " + binascii.hexlify(data[offset:offset+8]).decode('utf-8')  + " <= " + str(binascii.hexlify(q)))
